...
class VideoAIService:
    ...
    def _get_video_transcript(self, video_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                transcript_data = transcript.fetch()
                print(f"✅ Found auto-generated English transcript with {len(transcript_data)} segments")
                return transcript_data
            except:
                pass
            ...
            try:
                transcript = transcript_list.find_transcript(['en-US', 'en-GB', 'es', 'fr', 'de', 'it'])
                transcript_data = transcript.fetch()
                print(f"✅ Found transcript in other language with {len(transcript_data)} segments")
                return transcript_data
            except:
                pass
            ...
            print(f"📝 This could be due to: no transcript available, regional restrictions, or private video")
        
        # Return placeholder if no transcript found
        print(f"⚠️ No transcript available for video {video_id}")
        return [{"text": "Transcript not available for this video. This could be due to privacy settings, regional restrictions, or the creator not providing captions.", "start": 0, "duration": 0}]
    ...
    async def _generate_notes(self, video_context: VideoContext) -> str:
        ...
        transcript_with_times = self._format_transcript_for_analysis(
          ...
          for i, item in enumerate(transcript_data):
            try:
                ...
                if hasattr(item, 'text'):
                    ...
                    duration = getattr(item, 'duration', 5)
                else:
                    text = item['text']
                    start_time = item['start']
                ...
                if (start_time - current_start > 120) or i == len(transcript_data) - 1:
                    if segment_texts:
